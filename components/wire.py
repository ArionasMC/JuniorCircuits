...
from core.constants import COMPONENT_SIZE, LINE_THICKNESS
...
class Wire(pygame.sprite.Sprite):
    ...
    def update_surface(self):
        ...
        if self.right and self.down:
            ...
            offsets.append((-0.25, -0.25))
            print('r-d')
        if self.right and self.up:
            paths.append(self.corner_path)
            transforms.append((0.5, 0, 90))
            offsets.append((-0.25, 0.0))
            print('r-u')
        ...
        for path in paths:
            ...
            x = self.rect.w * transforms[index][0] + offsets[index][0] * LINE_THICKNESS 
            y = self.rect.h * transforms[index][1] + offsets[index][1] * LINE_THICKNESS 
